...
help() {
    help="$(cat <<EOF
Test x264, will gather stats such as file size, duration of first pass and second pass and put them in a csv.
Usage: 
    ./encoder.sh [options]
Example:
...
if [ "$PASS" -eq 1 ]; then
    FIRST_TIME=$(env time --format="Sec %e" bash -c " ${BASE} --muxer raw -o \"${OUTPUT}/${TYPE}/${TYPE}.h264\" -" 2>&1 | awk ' /Sec/ { print $2 }')
    SECOND_TIME=0
else
    FIRST_TIME=$(env time --format="Sec %e" bash -c " ${BASE} --pass 1 --stats \"${OUTPUT}/${TYPE}/${TYPE}.log\" -o /dev/null -" 2>&1 | awk ' /Sec/ { print $2 }')
    SECOND_TIME=$(env time --format="Sec %e" bash -c " ${BASE} --pass 2 --stats \"${OUTPUT}/${TYPE}/${TYPE}.log\" --muxer raw -o \"${OUTPUT}/${TYPE}/${TYPE}.h264\" -" 2>&1 | awk ' /Sec/ { print $2 }')   
fi

# Convert to mp4 then mkv due to errors sometimes going straight to mkv
ERROR=$(ffmpeg -y -hide_banner -loglevel error -i "${OUTPUT}/${TYPE}/${TYPE}.h264" -c copy "${OUTPUT}/${TYPE}/${TYPE}.mp4" 2>&1)
...
if [ -n "$ERROR" ] || [ -n "$ERROR2" ]; then
    rm -rf "${OUTPUT}/${TYPE}"
    die "x264 failed to encode ${TYPE}"
fi
...
SIZE=$(du -k "${OUTPUT}/${TYPE}/${TYPE}.h264" | awk '{print $1}')
BITRATE=$(ffprobe -i "${OUTPUT}/${TYPE}/${TYPE}.mkv" 2>&1 | awk ' /bitrate:/ { print $(NF-1) }')

rm -f "${OUTPUT}/${TYPE}/${TYPE}.log" &&
rm -f "${OUTPUT}/${TYPE}/${TYPE}.webm" &&
rm -f "${OUTPUT}/${TYPE}/${TYPE}.h264" &&
rm -f "${OUTPUT}/${TYPE}/${TYPE}.mp4" &&
rm -f "${OUTPUT}/${TYPE}/${TYPE}.log.cutree" 

echo -n "x264,${COMMIT},${PRESET},${INPUT_NAME},${SIZE},${TYPE},${BITRATE},${FIRST_TIME},${SECOND_TIME},${DECODE_TIME}" > "${OUTPUT}/${TYPE}/${TYPE}.stats"
