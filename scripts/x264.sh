...
Encoding Options:
    ...
    --preset     [number]   Set encoding preset, aomenc higher is faster, x4 lower is faster  (default 6)
    --pass       [number]   Set amount of passes                                                (default 1)
    --vbr                   Use vbr mode (applies to aomenc/x265/x264 only)
    --crf                   Use crf mode (applies to x264 only)                                 (default)
EOF
            )"
            echo "$help"
}
...
BASE="ffmpeg -y -hide_banner -loglevel error -i \"$INPUT\" -strict -1 -pix_fmt yuv420p -f yuv4mpegpipe - | x264 --log-level 0 --no-progress --demuxer y4m --preset ${PRESET} ${QUALITY_SETTINGS}"
...
if [ "$VBR" -ne -1 ] || [ "$PASS" -eq 1 ]; then
    ...
    SECOND_TIME=$(env time --format="Sec %e" bash -c " $BASE --pass 2 --stats \"$OUTPUT/${FOLDER}_$TYPE/${FOLDER}_$TYPE.log\" --muxer raw -o \"$OUTPUT/${FOLDER}_$TYPE/${FOLDER}_$TYPE.h264\" -" 2>&1 | awk ' /Sec/ { print $2 }')
else
    FIRST_TIME=$(env time --format="Sec %e" bash -c " $BASE --muxer raw -o \"$OUTPUT/${FOLDER}_$TYPE/${FOLDER}_$TYPE.h264\" -" 2>&1 | awk ' /Sec/ { print $2 }')
    SECOND_TIME=0
fi

ERROR=$(ffmpeg -y -hide_banner -loglevel error -i "${OUTPUT}/${FOLDER}_${TYPE}/${FOLDER}_${TYPE}.h264" -c copy "${OUTPUT}/${FOLDER}_${TYPE}/${FOLDER}_${TYPE}.mp4" 2>&1)
...
rm -f "$OUTPUT/${FOLDER}_$TYPE/${FOLDER}_$TYPE.h264" &&
...
echo -n "x264,${COMMIT},${PRESET},${INPUT_NAME},${SIZE},${TYPE},${BITRATE},${FIRST_TIME},${SECOND_TIME},${DECODE_TIME}" > "$OUTPUT/${FOLDER}_$TYPE/${FOLDER}_$TYPE.stats"
